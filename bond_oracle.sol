...
contract OlympusBondDepositoryOracle is Ownable {
    ...
    function bondPrice() public view returns ( uint price_ ) {
      ...
      if (priceUSD == 0) price_ = (terms.minimumPriceUSD).div(1e6).div(uint(assetPrice()));
      else if ( priceUSD < terms.minimumPriceUSD ) {
    ...
    function bondPriceInUSD(uint _bondPrice) public view returns ( uint price_ ) {
        price_ = _bondPrice.mul( uint( assetPrice() ) ).mul( 1e6 );
    }
